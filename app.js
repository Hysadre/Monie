...
async function saveToSupabase() {
  ...
  if(!error) { const d=$('save-indicator'); if(d){d.textContent='· Sauvegardé';d.style.color='#2ECBA1';setTimeout(()=>{d.textContent='';},2200);} }
}
...
function renderPatrimoine() {
  ...
  if(canvas){
    ...
    charts['chart-patrimoine']=new Chart(canvas,{type:'doughnut',data:{labels:cats.map(c=>c.l),datasets:[{data:cats.map(c=>Math.round(c.v)),backgroundColor:cats.map(c=>c.c),borderWidth:0}]},options:{responsive:false,plugins:{legend:{display:false}},cutout:'58%'}});
...
function renderChargesPie(){
  ...
  if(canvas){
    ...
    charts['chart-charges']=new Chart(canvas,{type:'doughnut',data:{labels:cats.map(c=>c.l),datasets:[{data:cats.map(c=>Math.round(c.v)),backgroundColor:cats.map(c=>c.c),borderWidth:0}]},options:{responsive:false,plugins:{legend:{display:false}},cutout:'58%'}});
...
function renderTransactions(){
  ...
  if(txPie){
    ...
    if(entries2.length){charts['chart-tx-pie']=new Chart(txPie,{type:'doughnut',data:{labels:entries2.map(e=>e[0]),datasets:[{data:entries2.map(e=>Math.round(e[1])),backgroundColor:entries2.map(e=>CAT_COLORS[e[0]]||'#7B7A8E'),borderWidth:0}]},options:{responsive:false,plugins:{legend:{display:false}},cutout:'58%'}});}
