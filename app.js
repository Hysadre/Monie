...
function initUI() {
  ...
  document.querySelectorAll('.month-nav button:last-child').forEach(b=>{b.style.opacity='0.3';b.style.cursor='not-allowed';});
  renderAll();
}
...
function changeMonth(dir) {
  ...
  const newM = curMonth + dir;
  const newY = curMonth + dir > 11 ? curYear + 1 : curMonth + dir < 0 ? curYear - 1 : curYear;
  const adjM = newM > 11 ? 0 : newM < 0 ? 11 : newM;
  // Block future months
  if (newY > maxY || (newY === maxY && adjM > maxM)) return;
  curMonth = adjM; curYear = newY;
  const ml = MONTHS[curMonth] + ' ' + curYear;
  ['dash-month-label','tx-month-label','badge-month-overview','badge-month-tx'].forEach(id=>set(id,ml));
  // Update month nav arrows
  const isCurrentMonth = curMonth === maxM && curYear === maxY;
  const nextBtns = document.querySelectorAll('.month-nav button:last-child');
  nextBtns.forEach(b => { b.style.opacity = isCurrentMonth ? '0.3' : '1'; b.style.cursor = isCurrentMonth ? 'not-allowed' : 'pointer'; });
