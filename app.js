...
function showImportPreview() {
  ...
  html += `<div style="margin-top:20px"><div class="card-title" style="margin-bottom:10px">✨ Aperçu des nouvelles transactions</div>`;
  const news = importPreviewData.filter(t => !t._duplicate).slice(0, 30);
  html += news.map(t => `
  ...
  `).join('');
  if (news.length > 30) html += `<div class="empty-sub" style="margin-top:8px">… et ${news.length - 30} autres</div>`;
  html += `</div></div>`;
  wrap.innerHTML = html;
