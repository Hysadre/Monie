...
function renderDashboard() {
  // Prévisionnel (budget paramétré dans Revenus/Charges)
  const prevRev = getRevTotal();
  const prevCharges = getChargesTotal();
  const prevSave = getSaveTotal();
  const prevSolde = prevRev - getChargesTotal() - prevSave;
  ...
  // ── KPI : toujours prévisionnel + indication réel si dispo ──
  set('kpi-rev', fmt(prevRev));
  set('kpi-rev-hint', hasTx ? 'Réel : ' + fmt(realEntrees) + ' saisis' : 'Budget paramétré');
  set('kpi-dep', fmt(prevCharges));
  set('kpi-dep-hint', hasTx ? 'Réel : ' + fmt(realSorties) + ' dépensés' : 'Prévisionnel hors épargne');
  set('kpi-save', fmt(prevSave));
  set('kpi-save-hint', prevRev > 0 ? Math.round(prevSave/prevRev*100) + '% du revenu' : '—');
  set('kpi-solde', (prevSolde < 0 ? '- ' : '') + fmt(prevSolde));
  set('kpi-solde-hint', hasTx ? 'Réel ce mois : ' + (realSolde>=0?'+':'- ') + fmt(realSolde) : 'Prévisionnel');
  const sc=$('kpi-solde-card');
  if(sc) sc.className='kpi '+(prevSolde>=0?'kpi-grad-green':'kpi-grad-red');

  // Badge mode
  const modeEl=$('dash-mode-badge');
  if(modeEl){
    if(hasTx){modeEl.textContent='✓ Données réelles disponibles';modeEl.style.background='rgba(46,203,161,0.12)';modeEl.style.color='#2ECBA1';}
  ...
  // Bar chart : prévisionnel avec ligne réelle si dispo
  const chartData = hasTx ? {
    labels:['Revenus','Charges','Épargne','Solde'],
    datasets:[
      {label:'Prévisionnel',data:[Math.round(prevRev),Math.round(prevCharges),Math.round(prevSave),Math.max(0,Math.round(prevSolde))],backgroundColor:['rgba(46,203,161,0.3)','rgba(255,107,107,0.3)','rgba(142,84,233,0.3)',prevSolde>=0?'rgba(79,142,247,0.3)':'rgba(123,122,142,0.3)'],borderRadius:7,borderSkipped:false,borderWidth:0},
    ...
    labels:['Revenus','Charges','Épargne','Solde'],
    datasets:[{data:[Math.round(prevRev),Math.round(prevCharges),Math.round(prevSave),Math.max(0,Math.round(prevSolde))],backgroundColor:['#2ECBA1','#FF6B6B','#8E54E9',prevSolde>=0?'#4F8EF7':'#7B7A8E'],borderRadius:7,borderSkipped:false,borderWidth:0}]
  };

  updateChart('chart-overview','bar',chartData,{
    plugins:{legend:{display:hasTx,labels:{color:'#7B7A8E',font:{size:10},boxWidth:10}}},
    scales:{
  ...
  if(dl){dl.innerHTML='';cats.slice(0,6).forEach(c=>{const d=document.createElement('div');d.className='legend-item';d.innerHTML=`<span class="legend-dot" style="background:${c.c}"></span><span style="flex:1">${c.l}</span><span style="font-family:var(--fm);font-size:11px;color:var(--mt)">${fmt(c.v)}</span>`;dl.appendChild(d);});}
  ...
  if(hl){
    const items=[
      ...
      {l:'Budget',v:prevSolde>=0?'+'+fmt(prevSolde):'-'+fmt(prevSolde),ok:prevSolde>=0,warn:prevSolde>=-100,hint:prevSolde>=0?'Équilibré ✓':'Dépassement'},
    ];
    hl.innerHTML='';
    items.forEach(i=>{
